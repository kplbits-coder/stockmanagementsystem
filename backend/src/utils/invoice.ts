...
export function generateInvoicePDF(data: InvoiceData, res: Response): void {
  ...
  doc
    ...
    .fontSize(28)
    .font('Helvetica-Bold')
    .text(data.companyName.toUpperCase(), 50, 50);

  doc
    .fillColor('#6b7280')
    .fontSize(10)
    .font('Helvetica')
    .text(data.companyTagline, 50, 85);
  ...
  if (data.panNo) {
    doc
      ...
      .fontSize(9)
      .font('Helvetica-Bold')
      .text(`PAN/VAT: ${data.panNo}`, 50, 100);
  }
  ...
  data.items.forEach((item, index) => {
    ...
    doc
      ...
      .text(`Rs. ${item.unitPrice.toFixed(2)}`, 340, y)
  ...
  doc
    ...
    .text(`Rs. ${data.subtotal.toFixed(2)}`, 480, y, { align: 'right', width: 65 });

  y += 18;
  doc
    .text('Tax:', totalsX, y)
    .text(`Rs. ${data.taxAmount.toFixed(2)}`, 480, y, { align: 'right', width: 65 });
  ...
  if (data.discount > 0) {
    ...
    doc
      ...
      .text(`-Rs. ${data.discount.toFixed(2)}`, 480, y, { align: 'right', width: 65 });
  }
  ...
  doc
    ...
    .text(`Rs. ${data.total.toFixed(2)}`, 480, y + 3, { align: 'right', width: 65 });
  ...
  if (data.paymentMethod) {
    ...
    if (data.amountPaid !== undefined) {
      doc.text(`Amount Paid: Rs. ${data.amountPaid.toFixed(2)}`, 350, y);
    }

    if (data.changeAmount && data.changeAmount > 0) {
      y += 14;
      doc.text(`Change: Rs. ${data.changeAmount.toFixed(2)}`, 50, y);
    }
  ...
  doc
    ...
    .text(`Generated by ${data.companyName}`, 50, 735, { align: 'center', width: 495 });
